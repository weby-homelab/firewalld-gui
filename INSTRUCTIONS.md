...
```nginx
server {
    listen 8649; # Порт, на якому буде доступна панель
    location / {
...
docker compose up -d
```
Панель буде доступна за адресою `http://IP_ВАШОГО_СЕРВЕРА:8649`.
...
4. Якщо все ОК — видаліть старий порт. Система не дасть видалити захищені порти (22, 80, 443, 8080, 8649), поки ви не завершите "Workaround".
